...
###proportions GCA-SCA in one bar##
  # Colors for components
  cols <- c(GCA_A = "#0C66C1", GCA_B = "#C71D0B", SCA = "#AAA9BF")
  
  splits_to_plot <- sort(unique(summary_gca_sca_ff$split))
  
  png("FF_barsGCAs_SCA_proportions.png", width = 9000, height = 6000, res = 600)
  
  # Layout: top row = panels; bottom row = one wide cell for X label + legend
  nP <- length(splits_to_plot)
  layout(rbind(1:nP, rep(nP + 1, nP)), heights = c(7.5, 0.7))
  
  par(mar = c(4.2, 5.5, 3.0, 0.6),  # panel margins
      oma = c(2.2, 4.5, 0.5, 0.5))  # outer margins (keep your size)
  par(mgp = c(3.8, 1.2, 0))         # axis title, labels, line
  par(xaxs = "i", yaxs = "i")
  
  for (j in seq_along(splits_to_plot)) {
    ss  <- splits_to_plot[j]
    tmp <- subset(summary_gca_sca_ff, split == ss)
    tmp <- tmp[order(tmp$meanDD), ]  # ensure dominance levels are ordered on x
    
    # Build matrix: rows = components, cols = dominance scenarios
    M <- t(as.matrix(tmp[, c("var_GCA_A", "var_GCA_B", "var_SCA")]))
    rownames(M) <- c("GCA_A", "GCA_B", "SCA")
    colnames(M) <- as.character(tmp$meanDD)
    
    # Convert to proportions (columns sum to 1)
    Mprop <- sweep(M, 2, colSums(M), "/")
    
    # Stacked bars of proportions (no 'main' here)
    barplot(Mprop, beside = FALSE,
            col = cols[rownames(Mprop)],
            ylim = c(0, 1), axes = FALSE,
            names.arg = colnames(Mprop),
            las = 1, cex.axis = 2.2, cex.names = 2.2)
    
    # Y-axis in decimals 0..1 (only on first panel)
    if (j == 1) {
      at <- seq(0, 1, by = 0.2)
      axis(2, at = at, labels = sprintf("%.1f", at), las = 1, cex.axis = 2.2)
    }
    
    # Panel title (Divergence ...) pushed a bit away from the plot
    mtext(paste0("Divergence ", ss, " generations"),
          side = 3, line = 1.2, cex = 2.0, font = 2)
  }
  
  # Bottom row: draw shared X label (upper part) and legend (below it) in the same row
  par(mar = c(0, 0, 0, 0))
  plot.new()
  # X-axis label (above)
  text(0.5, 0.75, "Dominance level", cex = 2.2)
  # Legend (below the label)
  legend(x = 0.5, y = 0.28, xjust = 0.5, yjust = 0.5,
         legend = names(cols), fill = unname(cols),
         horiz = TRUE, bty = "n", cex = 1.8, xpd = NA)
  
  # Outer Y label (moved further from axis)
  mtext("Proportion of genetic variance", side = 2, outer = TRUE, line = 1.0, cex = 1.9)
